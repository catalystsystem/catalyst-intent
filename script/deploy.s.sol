...
contract deploy is multichain {
    ...
    bytes32 inputSettlerSalt = 0x00000000000000000000000000000000000000000b1014d05f5714cab52d000c;
    ...
    function deploySettler(
        ...
        if (!isSettlerDeployed) {
            settler = new InputSettlerCompactLIFI{ salt: inputSettlerSalt }(COMPACT, initialOwner);
    ...
    function deployCompact() internal {
        ...
        if (!isCompactDeployed) {
            address compact =
                address(new TheCompact{ salt: 0x0000000000000000000000000000000000000000a762b8ea350d5a4d430100e0 }());
