...
contract ExclusiveOrder is IPreValidation, Ownable {
     ...
     */
    function validate(bytes32 key, address initiator) external view override returns (bool) {
        return (bytes12(key) == bytes12(0)) ? address(uint160(uint256(key))) == initiator || address(uint160(uint256(key))) == owner() : _allowList[key][initiator];
    }
