...
library BytesLib {
    ...
    function toAddress(bytes calldata _bytes, uint256 arg) internal pure returns (address tempAddress) {
        assembly {
            tempAddress := shr(96, calldataload(add(_bytes.offset, mul(0x20, arg))))
        }
    }
