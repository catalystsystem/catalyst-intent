...
struct CrossChainDescription {
    ...
    // then the order can be challenged successfully => Once claimed, the solver has block.timestamp + fillTime to fill the order.
    uint32 fillPeriod;
    // Period in seconds once after fillTime when the order can be optimistically claimed => Once claimed, the order can be challenged for until block.timestamp + fillTime + challengeTime.
    uint32 challengePeriod;
    // Period in seconds after a challenge has been submitted that the solver has to verify that they filled on the destination chain.
...
library OrderDescriptionHash {
    ...
    bytes constant CROSS_CHAIN_DESCRIPTION_TYPE =
        "CrossChainDescription(bytes32 destinationChain,uint256 minBond,uint32 fillPeriod,uint32 challengePeriod,uint32 proofPeriod,address[] approved Ambs)";
