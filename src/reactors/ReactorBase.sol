...
abstract contract ReactorBase is ISettlementContract {
    ...
    function orderKeyHash(OrderKey calldata orderKey) public returns (bytes32) {
        return _orderKeyHash(orderKey);
    }

    function _orderKeyHash(OrderKey calldata orderKey) internal virtual returns (bytes32);
