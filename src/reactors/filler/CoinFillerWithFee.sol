...
contract CoinFillerWithFee is CoinFiller, CanCollectGovernanceFee {
    ...
    constructor(address owner) payable CanCollectGovernanceFee(owner) { }
    ...
    function _getGovernanceTokens(address token) internal override view returns (uint256 amountTokens) {
        return SafeTransferLib.balanceOf(token, address(this));
    }

    function _resetGovernanceTokens(address token) internal override { }
}
