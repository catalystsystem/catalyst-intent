...
contract deployPolymer is multichain {
    ...
    ) public returns (PolymerOracle oracle) {
    ...
    ) public iter_chains(chains) broadcast returns (PolymerOracle oracle) {
    ...
    function deployPolymerOracle(
        address initialOwner,
        address polymerImplementation
    ) internal returns (PolymerOracle oracle) {
        ...
        if (!isOracleDeployed) return new PolymerOracle{ salt: 0 }(initialOwner, polymerImplementation);
        return PolymerOracle(expectedAddress);
    }

    function setPolymerConfig(
        PolymerOracle oracle,
        ...
        if (storedBlockChainid != 0 || storedChainIdentifier != 0) {
            ...
            revert PolymerOracle.AlreadySet();
        }
        // Set the map.
