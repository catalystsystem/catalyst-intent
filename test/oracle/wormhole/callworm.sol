...
contract TestWormholeCallWorm is Test {
  ...
  function test_compare_wormhole_implementation_and_calldata_version(bytes calldata message) view public {
    ...
    assertEq(
      parsedValidVm.payload, collectedMessage, "payload"
    );
    assertEq(
      message, collectedMessage, "payload"
    );
    assertEq(
      parsedValidVm.emitterChainId, remoteMessagingProtocolChainIdentifier, "emitterChainId"
    );
    assertEq(
      parsedValidVm.emitterAddress, remoteSenderIdentifier, "emitterAddress"
    );
  }
