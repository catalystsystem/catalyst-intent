...
library RegisterIntentLib {
    ...
    bytes32 constant STANDARD_ORDER_BATCH_COMPACT_TYPE_HASH =
        keccak256(
            bytes(
    ...
    function _validateChain(uint256 originChainId) internal view {
        // Check that this is the right originChain
        if (block.chainid != originChainId)
            revert WrongChain(block.chainid, originChainId);
    }
        ...
        if (setApprovals) {
            ...
            for (uint256 i; i < numInputs; ++i) {
                ...
                SafeTransferLib.safeApproveWithRetry(
                    EfficiencyLib.asSanitizedAddress(idAndAmount[0]),
                    address(COMPACT),
                    idAndAmount[1]
                );
            }
